...
router.get('/users', async (req, res) => {
  try {
    const result = await pool.query('SELECT nombres, apellidos, email, pais_id FROM registro_usuarios');
...
router.delete('/users/:email', async (req, res) => {
  ...
  try {
    const result = await pool.query('DELETE FROM registro_usuarios WHERE email = $1', [email]);
